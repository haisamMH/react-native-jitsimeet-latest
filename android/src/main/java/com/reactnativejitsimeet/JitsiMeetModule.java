...
public class JitsiMeetModule extends ReactContextBaseJavaModule {
  ...
  public void launchJitsiMeetView(ReadableMap options, Promise onConferenceTerminated) {
    ...
    URL serverURL;
    try {
      serverURL = new URL(options.hasKey("serverUrl") ? options.getString("serverUrl") : "https://meet.jit.si");
    ...
    JitsiMeetConferenceOptions builder = new JitsiMeetConferenceOptions.Builder()
        .setRoom(options.getString("room"))
        .setAudioOnly(options.getBoolean("audioOnly"))
    ...
    JitsiMeetActivity.launch(getReactApplicationContext(), builder);
    // JitsiMeetActivityExtended.launchExtended(getReactApplicationContext(),
    // builder);
  ...
  private void registerOnConferenceTerminatedListener(Promise onConferenceTerminated) {
    ...
    // .unregisterReceiver(onConferenceTerminatedReceiver);
    // }
    // };
    //
    // IntentFilter intentFilter = new
    // IntentFilter(BroadcastEvent.Type.CONFERENCE_TERMINATED.getAction());
    //
    // LocalBroadcastManager.getInstance(getReactApplicationContext())
    // .registerReceiver(this.onConferenceTerminatedReceiver, intentFilter);
  }
}
